...
export async function sendFriendRequest(authUserId: string, targetProfileId: string, email?: string) {
...
export async function listFriendRequests(authUserId: string, email?: string) {
  const me = await ensureProfileForAuthUser(authUserId, email);
...
export async function listFriends(authUserId: string, email?: string) {
  const me = await ensureProfileForAuthUser(authUserId, email);
  return listFriendProfiles(me.id);
}

export async function discoverProfiles(authUserId: string, email?: string, keyword?: string) {
  const me = await ensureProfileForAuthUser(authUserId, email);
  return listDiscoverProfiles(me.id, keyword);
}

export async function assertAreFriends(profileA: string, profileB: string) {
