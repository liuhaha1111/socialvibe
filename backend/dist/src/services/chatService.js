...
import { createMessage, ensureConversationMember, getLatestMessage, getOrCreateActivityConversation, getOrCreateDirectConversation, isConversationMember, listConversationMembers, listConversationsForProfile, listMessages } from "../repositories/chatRepository.js";
...
export async function ensureActivityGroupConversation(activityId, hostProfileId) {
...
export async function addProfileToActivityGroup(activityId, profileId) {
    const conversation = await getOrCreateActivityConversation(activityId);
    await ensureConversationMember(conversation.id, profileId, "member");
    return conversation;
}
export async function getOrCreateDirectConversationForAuthUser(authUserId, friendProfileId, email) {
...
export async function listConversationsForAuthUser(authUserId, email) {
    const me = await ensureProfileForAuthUser(authUserId, email);
...
export async function getConversationMessagesForAuthUser(authUserId, conversationId, email) {
    const me = await ensureProfileForAuthUser(authUserId, email);
...
export async function sendMessageForAuthUser(authUserId, conversationId, content, email) {
    const me = await ensureProfileForAuthUser(authUserId, email);
    if (!(await isConversationMember(conversationId, me.id))) {
        throw new AppError(403, "FORBIDDEN", "Not a conversation member");
    }
